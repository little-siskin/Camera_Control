...
class Visualizer():
    ...
    def draw_results(self, img, boxes, confs, classes):
        ...
        for box, conf, cls in zip(boxes, confs, classes):
            ...
            # draw text
            cv2.putText(final, '%s %f' % (cls_name, conf), org=(x1, int(y1+10)), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.5, color=(0, 0, 0))
        cv2.addWeighted(overlay, 0.5, final, 1 - 0.5, 0, final)
        cv2.imwrite('/home/o/zrl/yolov5-tensorrt-master/python/lib/result/_1.jpg', final)
        cv2.waitKey(20)
        return final
